...
- **On-device state handles** now embed a per-instance owner token: loading a handle from a different `Llama` instance raises `LlamaError` instead of risking a native crash. **Handles from v0.5.0 are not loadable in v0.6.0** (re-save in-session snapshots after upgrading; durable `get_state()` snapshots are unaffected).
- **Hardening**: per-call `n_draft_max` overrides validated to `[1, 8]`; `generate_stream` acquires its lock before any native context mutation; logprobs reset bumps the on-device state epoch; `LlamaPool` warmup keeps the prompt-cache mirror in sync.
- **Full details**: `docs/CHANGELOG-v0.6.0.md`.
...
- **On-device per-seq state**: `Context.save_seq_state_on_device(seq_id) -> bytes` / `load_seq_state_on_device(handle, dest_seq_id)` wrap `llama_state_seq_{get,set}_data_ext` with `LLAMA_STATE_SEQ_FLAGS_ON_DEVICE`. Short-lived in-session handles for fast branching; durable snapshots still use `get_state()`. (v0.6.0: handles embed a per-instance owner token — cross-instance loads are rejected.)
