...
**MTP (Multi-Token Prediction)** — `LlamaConfig.ctx_type=LLAMA_CONTEXT_TYPE_MTP` (1) selects the MTP graph variant for the user-facing context (used by `tests/test_mtp.py`). Setting `speculative=True` on `generate()` / `generate_stream()` / `create_chat_completion()` activates the draft-verify loop in `src/bindings/llama_cpp.cpp::generate_tokens_speculative_mtp` (linked via `libllama-common`), which uses a **dual-context architecture**: the user-facing context (`ctx_tgt`, DEFAULT graph) is the verifier, and an internal draft context (`ctx_dft`, MTP graph) is constructed against the same model when speculative is requested. The loop initializes a `common_speculative *` configured for `COMMON_SPECULATIVE_TYPE_DRAFT_MTP`, drafts up to `n_draft_max` (default 2, range [1, 8]) tokens per round via `common_speculative_draft`, decodes the `[id_last, drafted...]` batch on `ctx_tgt`, mirrors that batch onto `ctx_dft` via `common_speculative_process`, runs the standard sampler chain (and grammar, if any) at each position to verify, accepts matching drafts, and corrects on the first mismatch. Rejected drafts are trimmed via `kv_cache_seq_rm` on both contexts. **Off-by-1 invariant (load-bearing):** at loop exit the user-facing KV ends exactly 1 position *behind* the mirror — the final `corrected_id` is emitted but deliberately left undecoded to become next round's `id_last`. A following *speculative* turn re-decodes it in its first verify batch (self-heals). Do **not** force `KV == mirror` at loop exit (decode-forward or mirror-truncation both route the token through a second `common_speculative_process`, double-applying the draft's recurrent MTP state → garbage); the only safe exit reconciliation is trimming KV when it's *ahead* (accepted-but-unemitted drafts on EOS/stop). Matched stop tokens are erased from *both* `output` and `mirror` so they stay equal (this also prevents a `GGML_ASSERT(impl)` crash on the next speculative turn). **Mixed-mode guard:** switching modes mid-session (`speculative=True` then `False`, or vice-versa) on the same KV would produce drift/garbage. `Llama` tracks `_last_gen_speculative`; `_guard_speculative_mode_switch()` reconciles asymmetrically when a `reset_kv_cache=False` continuation crosses the boundary (applied in `_generate_from_tokens`, `generate_stream`, and the logprobs path; `_invalidate_prompt_cache` resets the tracker to None). **spec→nonspec:** the guard heals *in place*, preserving prefix reuse, with two exit shapes: KV *behind* the mirror (the off-by-1) → `decode_one` the undecoded tail (the emitted-but-undecoded `corrected_id`) so KV aligns with the mirror; KV *aligned* with the mirror (max_tokens ran out exactly on an accepted draft) → the logits buffer still points at the last verify batch's final *drafted* position (stale when any draft was rejected that round), so trim the tail token via `seq_rm` and re-decode it to refresh logits (falls back to a full reset when hybrid memory refuses the trim). **nonspec→spec:** the draft recurrent state can only rebuild from position 0 (a full prefix decode either way), so the guard forces `reset_kv_cache=True`. Same-mode continuations are untouched. Separately, `_reconcile_kv_ahead_of_mirror()` (run after every cached commit) clears KV when it ends *ahead* of the mirror — the hybrid-model case where a multi-token stop stranded prefix tokens that `seq_rm` couldn't safely trim; it never fires on the speculative off-by-1 (KV *behind*). RNG trajectory note: speculative advances the dist sampler's RNG once per *position* in each batch rather than once per *step*, so identical seeds give identical *distributions* but not bit-exact trajectories vs. the per-token path; greedy (`temperature=0.0`) outputs **are** identical. Preconditions: user-facing `ctx_type=LLAMA_CONTEXT_TYPE_DEFAULT` (validated by `Llama._validate_speculative`); model exposes an MTP graph (`Context::supports_speculative_mtp()`, gated on `<arch>.nextn_predict_layers > 0` metadata via `Context::mtp_predict_layers()` — **not** on MTP-context allocation success, which is not a capability signal (pre-b9180 llama.cpp allocates a degenerate MTP context on `qwen35`-arch checkpoints without MTP layers and the draft path then SIGABRTs; b9180+ rejects the allocation); see `docs/CHANGELOG-2026-06-03.md`); `config.embeddings=False`. `memory_can_shift()` is intentionally **not** required — MTP-capable hybrid models report `memory_can_shift()=False` for their DEFAULT context yet run speculative correctly because the loop trims rejected drafts via `n_rs_seq` recurrent-state rollback on the draft context (a different mechanism than KV shift). Do not add a `memory_can_shift` gate to `_validate_speculative`. Acceleration target: ≥ 1.10× tok/s on Qwen3.6-MoE (unsloth claims 1.15–1.2× MoE / 1.4–2.2× dense).

**On-device per-seq state** — `Context::save_seq_state_on_device(seq_id) -> nb::bytes` and `Context::load_seq_state_on_device(handle, dest_seq_id)` wrap `llama_state_seq_{get,set}_data_ext` with `LLAMA_STATE_SEQ_FLAGS_ON_DEVICE`. The Python wrapper prefixes every handle with a 12-byte envelope (`b"LCNH"` magic + uint64 little-endian state-epoch) and validates it on load: any KV-clearing op (`reset()`, `kv_cache_clear()`, `set_state_data()`, `load_state()`, `embed()`, `_apply_prefix_reuse` LCP trim, re-saving any seq) bumps `Llama._state_epoch` via `_invalidate_prompt_cache`, so a stale handle raises `LlamaError` instead of letting llama.cpp's `ggml_abort` terminate the process. Garbage handles (missing magic) raise `LlamaError` too. The validation is conservative — re-saving seq 1 invalidates a still-valid seq 0 handle — which trades a niche multi-seq capability for safety on the single-stream default. Mirror-only drops (`cache_prompt=False` mid-session, where KV is unchanged) use `_drop_mirror_only` and don't bump the epoch. Treat handles as short-lived in-session references; for durable snapshots use `get_state()`.
...
| `test_speculative_validation.py` | Speculative preconditions + metadata gate (positive case needs MTP model) |
| `test_speculative_mtp.py` | End-to-end draft-MTP speculative + mode-switch continuations — needs MTP model |
| `test_unified_speculative.py` | UnifiedLLM speculative auto/forced resolution — needs MTP model |

`conftest.py` provides the `model_path`/`test_model` fixtures and centralizes `MODEL_PATH`/`requires_model` and `MTP_MODEL_PATH`/`requires_mtp_model` (env overrides `LLAMA_TEST_MODEL` / `LLAMA_MTP_TEST_MODEL`) — import these instead of redefining per-file.
...
16. **Stale on-device state handle** — invalidated by `reset()` / `kv_cache_clear()` / `set_state_data()` / `load_state()` / `embed()` / LCP-trim / re-saving any seq. The Python wrapper validates the embedded epoch and raises `LlamaError` on stale or garbage-prefix handles, converting llama.cpp's `ggml_abort` into a recoverable Python exception. Use `get_state()` for durable snapshots.
...
```
On macOS: `brew install llama.cpp`. Update C++ bindings if the upstream API changed — in particular check `llama-ext.h`: its staging APIs are not stable (e.g. `llama_set_embeddings_pre_norm` → `llama_set_embeddings_nextn` in b9496 silently broke import of every built extension until rebuilt). After `make install`, always rebuild and import-test (`python -c "import llama_cpp"`).

## Reference

- API surface: `docs/API.md`
- Per-version changelogs: `docs/CHANGELOG-v0.3.6.md`, `docs/CHANGELOG-v0.4.0.md`, `docs/CHANGELOG-v0.5.0.md`, `docs/CHANGELOG-v0.6.0.md`, `docs/CHANGELOG-2026-05-02.md`, `docs/CHANGELOG-2026-05-27.md`, `docs/CHANGELOG-2026-06-03.md`, `docs/CHANGELOG-2026-06-12.md`
